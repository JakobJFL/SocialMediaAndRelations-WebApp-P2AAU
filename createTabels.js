...
con.connect(function(err) { 
	...
	sql = "CREATE TABLE `users` ( `user_id` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,`psw` varchar(50) NOT NULL,`fname` varchar(50) NOT NULL, `lname` varchar(50) NOT NULL,`mail` varchar(50) NOT NULL,`study` int(11),`birthDate` date NOT NULL,`bio` varchar(300),`REG_TIMESTAMP` timestamp NOT NULL DEFAULT current_timestamp(),`CHG_TIMESTAMP` datetime,`state` int(11) NOT NULL DEFAULT 1, FOREIGN KEY(study) REFERENCES studys(study_id) ON DELETE SET NULL);"; 
	con.query(sql, function (err, result) { 
		if (err) throw err; 
