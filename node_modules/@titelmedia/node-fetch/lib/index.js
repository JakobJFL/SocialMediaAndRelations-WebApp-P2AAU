...
function getTypeOfBody(body) {
	if (body == null) {
		...
		return "Stream";
	} else if (body instanceof webStreamsPolyfill.ReadableStream ||
...
function createReadableStream(instance) {
	...
	if (bodyType === 'ReadableStream') {
		return body.pipeThrough(new webStreamsPolyfill.TransformStream({
	...
	const readable = new webStreamsPolyfill.ReadableStream({
		start(controller) {
			switch (bodyType) {
				case "String":
					// body is a string:
